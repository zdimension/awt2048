 ...
 */
class JeuCanvas extends Canvas
{
    /**
     ...
     */
    private static final Color[] COULEURS_TEXTE = {
     ...
     */
    private static final int MARGE_EXTERIEURE = 10;
    /**
     * Couleur de fond de la fenêtre
     */
    private static final Color COULEUR_FOND = new Color(250, 248, 239);
    /**
     * Couleur de fond de la grille de jeu
     */
    private static final Color COULEUR_FOND_GRILLE = new Color(187, 173, 160);
    /**
     * Fenêtre parent
     */
     ...
     */
    private static final Dimension SCOREBOX_SIZE = new Dimension(100, 62);
    /**
     ...
     */
    private static final int BUTTON_HEIGHT_SEC = BUTTON_HEIGHT * 3 / 4;
    /**
     * Hauteur de l'en-tête de la fenêtre
     */
    private static final int HAUTEUR_ENTETE = MARGE_EXTERIEURE * 3 + SCOREBOX_SIZE.height + BUTTON_HEIGHT + BUTTON_HEIGHT_SEC;
    /**
    ...
    private final List<JeuCanvasBouton> boutons;
    ...
    public void setModel(Jeu2048 model)
    {
        ...
        this.model.addObserver((Observable o, Object arg) ->
        {
            repaint();
            ...
            try
            {
                ModelIO.enregistrer("_auto.sav", this.model.getBackend());
            }
            catch (Exception e)
            {
     ...
     */
    JeuCanvas(Frame parent, Jeu2048 model)
    {
        this.parent = parent;
        this.setModel(model);
        ...
        addKeyListener(new KeyAdapter()
        {
            ...
            public void keyPressed(KeyEvent e)
            {
                ...
                {
                    case KeyEvent.VK_UP:
                        JeuCanvas.this.model.decaler(Jeu2048.HAUT);
                        break;

                    case KeyEvent.VK_DOWN:
                        JeuCanvas.this.model.decaler(Jeu2048.BAS);
                        break;

                    case KeyEvent.VK_LEFT:
                        JeuCanvas.this.model.decaler(Jeu2048.GAUCHE);
                        break;

                    case KeyEvent.VK_RIGHT:
                        JeuCanvas.this.model.decaler(Jeu2048.DROITE);
                        break;

                    case KeyEvent.VK_R:
                        JeuCanvas.this.model.nouveauJeu();
                        break;
        ...
        addMouseMotionListener(new MouseMotionAdapter()
        {
            ...
            public void mouseMoved(MouseEvent e)
            {
                ...
                for (JeuCanvasBouton btn : boutons)
                {
    ...
    {
        return COULEURS[valeur == 0 ? 0 : (int) Math.floor(Math.log(valeur) / Math.log(2))];
    }
    ...
    private void drawGrid(Graphics g, int yOff)
    {
        ...
        var marginIn = Math.min(width, height) / 32;
        ...
        for (var i = 0; i < model.getNbLignes(); i++)
        {
            for (var j = 0; j < model.getNbCols(); j++)
            {
                ...
                if (valeur != 0)
                {
                    ...
                    int taille = (int) (Math.pow(0.8, Math.floor(Math.log10(valeur))) * Math.min(wc, hc) / 1.75);
